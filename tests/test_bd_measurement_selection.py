...
class BdMeasurementSelectionTests(unittest.TestCase):
    ...
    def test_frontend_selection_uses_hidden_measurement_row_ids(self):
        ...
        self.assertIn("const rowIds = clickedPlotSelection(event)", script)
        self.assertIn("function clickedPlotSelection(event)", script)
        self.assertIn("state.selectedRowIds = rowIds", script)
