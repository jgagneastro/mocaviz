...
query_e = """
    SELECT mo.designation, mv.moca_aid, mv.moca_mtid, cspt.spectral_type AS spt, dr3.RUWE AS dr3_ruwe, mv.moca_oid, xyz.x_pc AS x, xyz.y_pc AS y, xyz.z_pc AS z, cbsd.x_opt, cbsd.y_opt, cbsd.z_opt
    ...
    LEFT JOIN (
        SELECT cbs.id, cbs.moca_oid, cbs.max_observables, cbs.moca_bsmdid
        FROM calc_banyan_sigma cbs
        JOIN moca_banyan_sigma_models mbsm ON mbsm.moca_bsmdid = cbs.moca_bsmdid
        WHERE mbsm.adopted = 1
    ) cbs ON cbs.moca_oid = mv.moca_oid AND cbs.max_observables = 1
